...
'This is the function we will use to add the new data
Public Function add()
'The first thing we have to do is make sure that the user has entered all the data
'as some databases are designed so that no field can contain a null value
'to do this, we just need to check that the text boxes all have data in them
'If they havent, we will throw up an error message and stop the function
...
rs.AddNew
'then we need to add the data into the required fields in the database
rs("surname") = txtSurname.Text
rs("Forename") = txtForename.Text
rs("phone_no") = txtPhone.Text
'then we need to update the database so that we can view the data
rs.Update
'now we can blank out all of the text fields
txtSurname.Text = vbNullString
txtSurname.Enabled = False
txtForename.Text = vbNullString
txtForename.Enabled = False
...
cmdAdd.Enabled = True
cmdend.Enabled = True
'now we need to call the list function so that our new surname appears
'in our list box
list
...
'This function will be used to edit existing data within the database
'We need to enable the textboxes so that the user can edit the data
txtSurname.Enabled = True
txtForename.Enabled = True
txtPhone.Enabled = True
'We also need to disable all the command buttons, so that the user cannot
...
cmdsave.Enabled = True
cmdcancel.Enabled = True
'Now we set dbedit = true so that VB Knows what we are trying to do
dbedit = True
...
'This is the function that we will use to edit the database
'The first thing we have to do is make sure that the user has entered all the data
'as some databases are designed so that no field can contain a null value
'to do this, we just need to check that the text boxes all have data in them
'If they havent, we will throw up an error message and stop the function
...
'Now we need to tell the database that we intend to edit already existing data
'in the database
rs.edit
'then we need to add the edited data into the required fields in the database
rs("surname") = txtSurname.Text
rs("Forename") = txtForename.Text
rs("phone_no") = txtPhone.Text
'then we need to update the database so that we can view the data
...
cmdAdd.Enabled = True
cmdend.Enabled = True
'next, we will need to set the recordset to the entire table, rather than just
'the one we have edited
Set rs = db.OpenRecordset("tbldata", dbOpenTable)
'now we need to call the list function so that our new surname appears
'in our list box
list
...
'If the user changes their mind about editing or adding data, we need to set the
'form back to its original state
txtSurname.Text = vbNullString
txtSurname.Enabled = False
txtForename.Text = vbNullString
txtForename.Enabled = False
...
Else
    Set rs = db.OpenRecordset("SELECT * FROM tbldata WHERE Surname LIKE '" & txtSearch.Text & "'" & "& '*'")
End If
